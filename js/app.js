...
function renderProjects(items) {
  ...
  el.innerHTML = items.map(p => `
    <article class="dynamic-card project-card">
      ${safe(p.photo) ? `<img class="content-image project-image" src="${p.photo}" alt="${p.name}">` : ""}
...
function renderServices(items) {
  ...
  el.innerHTML = items.map(s => `
    <article class="soft-card dynamic-card service-card">
      ${
        safe(s.photo)
          ? `<img class="content-image service-image" src="${s.photo}" alt="${s.name}">`
...
function renderEquipment(items) {
  ...
  el.innerHTML = items.map(e => `
    <article class="dynamic-card equipment-card">
      ${
        safe(e.photo)
          ? `<img class="content-image equipment-photo" src="${e.photo}" alt="${e.name}">`
          : `<div class="equipment-fallback">◈</div>`
      }
...
function renderPublications(items) {
  ...
  el.innerHTML = items.map(p => `
    <article class="dynamic-card publication-card">
      ${safe(p.photo) ? `<img class="content-image publication-image" src="${p.photo}" alt="${p.name}">` : ""}
